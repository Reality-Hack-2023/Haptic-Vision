...
public static class HxShared {
  ...
  //! @param [out] v2d The converted vector.
  public static void HxFromUnity(Vector2 v2, ref HaptxApi.Vector2D v2d) {
    if (v2d == null) {
      return;
    }
    v2d.setAll(v2.x, v2.y);
  }
  ...
  //! @param [out] v3d The converted vector.
  public static void HxFromUnity(Vector3 v3, ref HaptxApi.Vector3D v3d) {
    if (v3d == null) {
      return;
    }
    v3d.setAll(v3.z, -v3.x, v3.y);
  }
  ...
  //! @param [out] v3d The converted vector.
  public static void HxFromUnityScale(Vector3 unity_scale, ref HaptxApi.Vector3D v3d) {
    if (v3d == null) {
      return;
    }
    v3d.setAll(unity_scale.z, unity_scale.x, unity_scale.y);
  }
  ...
  //! @param [out] q The converted quaternion.
  public static void HxFromUnity(Quaternion uq, ref HaptxApi.Quaternion q) {
    if (q == null) {
      return;
    }
    q.setAll(-uq.w, uq.z, -uq.x, uq.y);
  }

  //! Convert a Unity Matrix4x4 to a HaptxApi.Transform.
  //!
  //! @param um The unity matrix to convert.
  //! @param [out] m The converted transform.
  public static void HxFromUnity(Matrix4x4 um, ref HaptxApi.Transform m) {
    if (m == null) {
      return;
    }
    m.setAllColumnMajor(um[10], -um[8], um[9], -um[2], um[0], -um[1], um[6], -um[4], um[5], um[14],
        -um[12], um[13]);
  }
  ...
  //! @param [out] m The converted transform.
  public static void HxFromUnity(Transform unityTransform, ref HaptxApi.Transform m) {
    HxFromUnity(unityTransform.localToWorldMatrix, ref m);
  }
  ...
  //! @param [out] haptxAngularVelocity The converted left-handed angular velocity vector.
  public static void HxFromUnityAngularVelocity(Vector3 unityAngularVelocity,
      ref HaptxApi.Vector3D haptxAngularVelocity) {
    // The negation on the following line is very necessary for angular velocities!
    HxFromUnity(-unityAngularVelocity, ref haptxAngularVelocity);
  }
