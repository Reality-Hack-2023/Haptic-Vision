...
public class HxPatch : MonoBehaviour, HxCore.IHxUpdate {
  ...
  public void Initialize() {
    ...
    ISocket socketParent = GetComponentInParent<ISocket>();
    ...
    RelDir relDir;
    string subbedLocatingFeature = _locatingFeature;
    if (socketParent.TryGetRelativeDirection(out relDir)) {
      subbedLocatingFeature = HxShared.SubstituteRelDir(_locatingFeature, relDir);
    }

    Matrix4x4 wLocatingFeature;
    if (!socketParent.TryGetLocatingFeatureTransform(subbedLocatingFeature,
  ...
  void RegisterTactors() {
    ...
    PeripheralLink peripheralLinkParent = GetComponentInParent<PeripheralLink>();
    if (peripheralLinkParent == null) {
      HxDebug.LogError(
          "HxPatch.RegisterTactors(): Owner doesn't implement PeripheralLink.",
          this);
      HardDisable();
      return;
    }

    if (peripheralLinkParent.Peripheral == null) {
      HxDebug.LogError(
          "HxPatch.RegisterTactors(): Null peripheral from owner.", this);
      HardDisable();
      return;
    }
    _peripheralId = peripheralLinkParent.Peripheral.id;

    ISocket socketParent = GetComponentInParent<ISocket>();
    if (socketParent == null) {
      HxDebug.LogError(
          "HxPatch.RegisterTactors(): Owner doesn't implement HxPatch.ISocket.", this);
      HardDisable();
      return;
    }
    ...
    if (!socketParent.TryGetLocatingFeatureTransform(locatingFeature, out wLocatingFeature)) {
      HxDebug.LogError(string.Format(
          "HxPatch.Initialize(): Failed to get transform for locating feature {0}.",
    ...
    long ciBodyId;
    if (!socketParent.TryGetCiBodyId(this, out ciBodyId)) {
      return;
    }

    foreach (HaptxApi.Tactor tactor in peripheralLinkParent.Peripheral.tactors) {
      if (string.Equals(subbedLocatingFeature, tactor.parent.getText(),
       ...
       core.ContactInterpreter.registerTactor(peripheralLinkParent.Peripheral.id, tactor,
  ...
  void InformContactInterpreterOfResult(Vector3 wTraceDirection, RaycastHit objectTraceResult, TactorData tactorDatum, long ciObjectId, Vector3 wTraceOriginPosM) {
    ...
    HxShared.HxFromUnity(wTraceDirection, ref hxTraceDirection);
    HaptxApi.Vector3D hxObjectHitPoint = HxReusableObjectPool<HaptxApi.Vector3D>.Get();
    HxShared.HxFromUnity(objectTraceResult.point, ref hxObjectHitPoint);
    HaptxApi.Vector3D hxNormal = HxReusableObjectPool<HaptxApi.Vector3D>.Get();
    HxShared.HxFromUnity(objectTraceResult.normal, ref hxNormal);
    HaptxApi.Vector2D hxUV1 = HxReusableObjectPool<HaptxApi.Vector2D>.Get();
    HxShared.HxFromUnity(objectTraceResult.textureCoord, ref hxUV1);
    HaptxApi.Vector2D hxUV2 = HxReusableObjectPool<HaptxApi.Vector2D>.Get();
    HxShared.HxFromUnity(objectTraceResult.textureCoord2, ref hxUV2);
    ...
    HxReusableObjectPool<HaptxApi.Vector2D>.Release(hxUV1);
    HxReusableObjectPool<HaptxApi.Vector2D>.Release(hxUV2);
  ...
  //! This interface is a means for @link HxPatch HxPatches @endlink to discover which 
  //! movables they're supposed to load.
  public interface ISocket {
    ...
    bool TryGetLocatingFeatureTransform(string locatingFeature, out Matrix4x4 wTransform);

    //! Get the relative direction the patch should use.
    //!
    //! @param [out] relDir Populated with the relative direction the patch should use.
    //! @returns True if @p relDir is successfully populated.
    bool TryGetRelativeDirection(out RelDir relDir);

    //! Get the HaptxApi::ContactInterpreter body ID of the attach parent.
    //!
    //! @param patch The patch calling this function.
    //! @param[out] bodyId The body ID associated with the parent the patch is attached to. Null
    //! if not associated.
    //!
    //! @returns Whether a body ID was found.
    bool TryGetCiBodyId(HxPatch patch, out long bodyId);
  }
}
