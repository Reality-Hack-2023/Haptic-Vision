...
public class HxHand : NetworkBehaviour, PeripheralLink, HxPatch.ISocket, HxCore.IHxUpdate {
  ...
  bool _touchPadWasPressed = false;
  ...
  bool TryGetHandLocationAndRotation(ref Matrix4x4 wMcp3, ref Matrix4x4 wVive) {
    if (core == null || _glove == null || !HaptxApi.OpenvrWrapper.isReady()) {
      return false;
    }
    ...
    if (!_glove.is_simulated) {
      ...
      _lastOpenvrWrapperReturnCode = HaptxApi.OpenvrWrapper.getTrackerTransform(
          Peripheral.vive_trackers.First.Value.serial, ref ovr);
      ...
      if (success) {
        wMcp3 = HxShared.UnityFromHx(ovr.operator_mult(
            Peripheral.vive_trackers.First.Value.transform.inverse()));
      }
    } else {
    ...
    if (success) {
      ...
      if (mocapOrigin != null) {
        wMcp3 = mocapOrigin.transform.localToWorldMatrix * wMcp3;
        wVive = mocapOrigin.transform.localToWorldMatrix * wVive;
      }
    }
  ...
  void UpdateHandAnimation() {
    ...
    HaptxApi.MocapFrame mocapFrameOriginal = new HaptxApi.MocapFrame();  // Directly from hardware
    HaptxApi.MocapFrame mocapFrame = new HaptxApi.MocapFrame();  // Mocap adjusted for compensators
    Matrix4x4 wMcp3 = wMcp3Original;  // MCP3 placement adjusted for compensators
    HaptxApi.AnimFrame animFrame = new HaptxApi.AnimFrame();  // Hand animation information
    ...
    if (_glove.is_simulated) {
      ...
      if (HaptxApi.OpenvrWrapper.isReady()) {
        bool touchPadWasPressed = false;
        if (HaptxApi.OpenvrWrapper.getControllerButtonPressed(hand.ToHx(),
            HaptxApi.ControllerButton.TOUCH_PAD, ref touchPadWasPressed) ==
            HaptxApi.OpenvrWrapper.ReturnCode.SUCCESS &&
            touchPadWasPressed && !_touchPadWasPressed) {
          float touchPadValue = 0.0f;
          if (HaptxApi.OpenvrWrapper.getControllerAxisValue(hand.ToHx(),
              HaptxApi.ControllerAxis.TOUCH_PAD_X, ref touchPadValue) ==
              HaptxApi.OpenvrWrapper.ReturnCode.SUCCESS) {
            if (touchPadValue > 0.0f) {
        ...
        _touchPadWasPressed = touchPadWasPressed;

        float triggerValue = 0.0f;
        if (HaptxApi.OpenvrWrapper.getControllerAxisValue(hand.ToHx(), HaptxApi.ControllerAxis.TRIGGER,
            ref triggerValue) == HaptxApi.OpenvrWrapper.ReturnCode.SUCCESS) {
      ...
      animFrame = _lastSimulatedAnimFrame;
      ...
      if (enableThimbleCompensation) {
        HaptxApi.ThimbleCompensator.applyToMocapFrame(mocapFrame,
      ...
      if (handAnimOptimizationMode == HandAnimationOptimizationMode.DYNAMIC &&
          ...
          mocapFrame, ref _avatarAnimOptimizedProfile)) {
        HxDebug.LogError("HxHand.updateHandAnimation(): Failed to optimize hand animation.", this);
        animProfile = _userProfile;
      }

      HaptxApi.DefaultHandIk.addToAnimFrame(_glove, mocapFrame, _userProfile, animProfile, animFrame);
    }

    if (motionCaptureVisualizer.visualize) {
      // Mocap visualizer shows unadjusted values.
      VisualizeMocapData(mocapFrameOriginal, wMcp3Original, wVive);
    }

    if (handAnimationVisualizer.visualize) {
      VisualizeHandAnimation(animFrame, animProfile, wMcp3);
    }
    ...
    for (int f_i = 0; f_i < (int)HaptxApi.Finger.F_LAST; f_i++) {
      for (int fj_i = 0; fj_i < (int)HaptxApi.FingerJoint.FJ_LAST; fj_i++) {
        ...
        if (animFrame.l_orientations.ContainsKey(joint)) {
          lJointOrients[flatIndex] = HxShared.UnityFromHx(animFrame.l_orientations[joint]);
        } else {
    ...
    if (collision.rigidbody != null) {
      if (collision.impulse.sqrMagnitude > 0.0f) {
        ...
        if (boneData == null || !boneData.hasGdBodyId) {
          ...
          HxShared.HxFromUnity(contactLocation, ref hxContactLoaction);
  ...
  public void SetScale(float scale) {
    ...
    if (wCompScale == 0.0f) {
      ...
      for (int f_i = 0; f_i < (int)HaptxApi.Finger.F_LAST; f_i++) {
        ...
        HxShared.HxFromUnity(mcp3Joint1, ref hxMcp3Joint1);
  ...
  public bool TryGetRelativeDirection(out RelDir relDir) {
    relDir = hand;
    return true;
  }
    ...
    if (Peripheral != null) {
      ...
      if (Peripheral.is_simulated) {
        ...
        if (Peripheral.vive_trackers.Count > 0) {
          HaptxApi.ViveTracker viveTracker = Peripheral.vive_trackers.First.Value;
        ...
        if (Peripheral.hyleas_sources.Count > 0) {
          HaptxApi.HyleasSource hyleasSource = Peripheral.hyleas_sources.First.Value;
