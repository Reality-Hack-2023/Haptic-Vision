...
public class HxPlayer : NetworkBehaviour {
  ...
  //! The tracked pose driver for the player's HMD. Only enabled if this is a local player.
  [SerializeField]
  [Tooltip("The tracked pose driver for the player's HMD. Only enabled if this is a local player.")]
  private TrackedPoseDriver _hmdTrackedPoseDriver = null;
  ...
  void Reset() {
    ...
    _hmdTrackedPoseDriver = GetComponentInChildren<TrackedPoseDriver>();
#endif
  }

  void Awake() {
    if (_leftHandPrefab != null) {
      HxVersionCompatability.MirrorClientRegisterPrefab(_leftHandPrefab);
    }
    if (_rightHandPrefab != null) {
      HxVersionCompatability.MirrorClientRegisterPrefab(_rightHandPrefab);
    }
  }
  ...
  public override void OnStartClient() {
    ...
    if (_hmdTrackedPoseDriver != null) {
      _hmdTrackedPoseDriver.enabled = hasAuthority;
    }
  }
  ...
  public override void OnStartServer() {
    if (_leftHandPrefab != null) {
      ...
      hand.mocapOriginObject = gameObject;
