...
public class HxCore : MonoBehaviour {
  ...
  //! The HsvController being used to render peripherals not connected to an Air Controller.
  private HaptxApi.HsvController _standaloneHardwareHsv = null;
  ...
  void Awake() {
    ...
    if (HxSteamVRSettings.Instance.setRequiredSteamVRSettingsOnStart &&
        Valve.VR.SteamVR.settings.lockPhysicsUpdateRateToRenderFrequency) {
      Valve.VR.SteamVR.settings.lockPhysicsUpdateRateToRenderFrequency = false;

      if (!HxSteamVRSettings.Instance.suppressChangingSteamVRSettingsWarning) {
        string message =
            "SteamVR plugin setting \"Lock Physics Update Rate to Render Frequency\" was " +
            "set to false, as the HaptX plugin behaves poorly with this setting on." +
            "\nThis warning can be suppressed in the HaptX settings window.";
  ...
  //! Helper function to render the relevant peripherals from a map of haptic frames 
  //! onto an Air Controller.
  //! 
  //! @param airController The Air Controller to render onto.
  //! @param hapticFrames The map of Peripheral ids to haptic frames.
  //! @param[out] pneumaticFrameOut The PneumaticFrame rendered onto the Air Controller.
  //! 
  //! @returns True if pneumaticFrameOut was succesfully populated with data.
  private bool renderAirController(
      HaptxApi.AirController airController,
      UnorderedHaptxUuidToHapticFrame hapticFrames, HaptxApi.PneumaticFrame pneumaticFrameOut) {
    HaptxApi.PneumaticFrame pneumaticFrame = pneumaticFrameOut;
    pneumaticFrame.pressures_pa.Clear();
    using (var peripheralFromSlotPoolObject = HxReusableObjectPool<IntToPeripheral>.GetWrapped()) {
      IntToPeripheral peripheralFromSlot = peripheralFromSlotPoolObject.ReusableObject;
  ...
  void Update() {
    ...
    foreach (HaptxApi.Dk2AirController airController in _haptxSystem.getDk2AirControllers()) {
      ...
      using (var pneumaticFramePoolObject = HxReusableObjectPool<HaptxApi.PneumaticFrame>.GetWrapped()) {
        HaptxApi.PneumaticFrame pneumaticFrame = pneumaticFramePoolObject.ReusableObject;
        if (renderAirController(airController, hapticFrames, pneumaticFrame)) {
          ...
          if (airController.getId(ref airControllerId) ==
            ...
            if (_hsvControllerFromAirControllerId.TryGetValue(airControllerId, out hsvController) &&
                hsvController != null) {
              HaptxApi.AirController.ReturnCode ret = hsvController.render(pneumaticFrame);
              if (ret != HaptxApi.AirController.ReturnCode.SUCCESS) {
                HxDebug.LogError(string.Format(
    ...
    if (_standaloneHardwareHsv != null) {
      using (var pneumaticFramePoolObject = HxReusableObjectPool<HaptxApi.PneumaticFrame>.GetWrapped()) {
        renderAirController(_standaloneHardwareHsv, hapticFrames, pneumaticFramePoolObject.ReusableObject);
      }
    }
    if (_simulatedHardwareHsv != null) {
      using (var pneumaticFramePoolObject = HxReusableObjectPool<HaptxApi.PneumaticFrame>.GetWrapped()) {
        renderAirController(_simulatedHardwareHsv, hapticFrames, pneumaticFramePoolObject.ReusableObject);
      }
    }
  ...
  bool InitializeHaptxSystem() {
    ...
    HashSet<string> hsvRegisteredPeripheralIds = new HashSet<string>();
    ...
    if (_haptxSystem.getDk2AirControllers() != null && _haptxSystem.getHsvControllers() != null) {
      ...
      foreach (HaptxApi.HsvController hsvController in _haptxSystem.getHsvControllers()) {
        ...
        if (airControllerNode == null) {
          // We're out of Dk2AirControllers to mirror, see if any gloves directly plugged in 
          // through the list of hyleas systems.
          foreach (HaptxApi.HyleasSystem hyleasDevice in _haptxSystem.getHyleasSystems()) {
            HaptxApi.Glove glove = hyleasDevice.getGlove();
            if (!hsvRegisteredPeripheralIds.Contains(glove.id.toString())) {
              if (_standaloneHardwareHsv == null) {
                _standaloneHardwareHsv = hsvController;
              }

              int slot = glove.handedness == HaptxApi.RelativeDirection.RD_LEFT ? 1 : 2;
              ret = _standaloneHardwareHsv.attachPeripheral(slot, glove);
              if (ret != HaptxApi.AirController.ReturnCode.SUCCESS) {
                HxDebug.LogError(
                    "HsvController.attachPeripheral() failed with return code "+
                    $"{(int)ret}: {HaptxApi.AirController.toString(ret)}");
              }
            }
          }

          if (_standaloneHardwareHsv != hsvController) {
            // We've run out of Dk2AirControllers to mirror and have no remaining spare gloves. 
            // We can use this last hsv_controller to render simulated hardware.
        ...
        peripheralFromSlot.Clear();
        ret = airControllerNode.Value.getAttachedPeripherals(peripheralFromSlot);
        if (ret != HaptxApi.AirController.ReturnCode.SUCCESS) {
          HxDebug.LogError(string.Format(
          ...
          foreach (var keyValue in peripheralFromSlot) {
            ...
            if (ret == HaptxApi.AirController.ReturnCode.SUCCESS) {
              hsvRegisteredPeripheralIds.Add(keyValue.Value.id.toString());
            } else {
              HxDebug.LogError(string.Format(
