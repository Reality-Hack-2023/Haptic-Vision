...
public abstract class HxDirectEffect : HxHapticEffect {
  ...
  public bool AddToCoverageRegion(string coverageRegion) {
    ...
    IEnumerable<HxHand> hxHands = FindObjectsOfType<HxHand>();
    bool aTactorHasFailed = false;
    foreach (HxHand hxHand in hxHands) {
      if (hxHand != null && hxHand.Peripheral != null) {
        foreach (HaptxApi.Tactor tactor in hxHand.Peripheral.tactors) {
          if (tactor.coverage_region.operator_comp_eq(coverageRegionName) &&
              !AddToTactor(hxHand.Peripheral.id, tactor.id)) {
    ...
    if (coverageRegions.Remove(coverageRegion)) {
      ...
      foreach (HxHand hxHand in hxHands) {
        if (hxHand != null && hxHand.Peripheral != null) {
          foreach (HaptxApi.Tactor tactor in hxHand.Peripheral.tactors) {
            if (tactor.coverage_region.operator_comp_eq(coverageRegionName)) {
              RemoveFromTactor(hxHand.Peripheral.id, tactor.id);
            }
          }
        }
      }
