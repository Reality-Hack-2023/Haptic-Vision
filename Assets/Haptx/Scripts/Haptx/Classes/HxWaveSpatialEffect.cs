...
//! @brief An implementation of HxSpatialEffect that defines the Haptic Effect using a simple sine
//! wave generator. The waveform starts at 0 then increases by default.
//!
//! @ingroup group_unity_plugin
public class HxWaveSpatialEffect : HxSpatialEffect {
  ...
  //! Default constructor.
  public HxWaveSpatialEffect() : base() {
    _isLooping = true;
  }
  ...
  protected override float GetForceN(HaptxApi.SpatialEffect.SpatialInfo spatialInfo) {
    float waveOutputN = HxShared.EvaluateSineWave(spatialInfo.time_s, frequencyHz,
        inputPhaseOffsetDeg);
