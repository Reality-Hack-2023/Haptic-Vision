...
//! An implementation of HxObjectEffect that defines the Haptic Effect using a provided
//! AnimationCurve.
//!
//! @ingroup group_unity_plugin
public class HxCurveObjectEffect : HxObjectEffect {
  ...
  protected override float GetForceN(HaptxApi.ObjectEffect.ContactInfo contactInfo) {
    if (_forceCurve == null) {
      ...
      float curveOutputN = _forceCurve.Evaluate((_inputTimeScale > 0.0f ? _minTimeS : _maxTimeS)
          + _inputTimeScale * contactInfo.time_s);

      return -contactInfo.object_normal.dot(contactInfo.body_normal) *
          _outputForceScale * curveOutputN;
    }
  }
