...
public class HxGlobalSettingsWindow : EditorWindow {
  ...
  //! A reference to serialized HxSteamVRSettings.
  private static SerializedObject _serializedSteamVRSettings = null;
  ...
  private void OnGUI() {
    ...
    if (_serializedSteamVRSettings == null) {
      if (HxSteamVRSettings.Serialized == null) {
        HxDebug.Log(
          $"Creating a serialized {nameof(HxSteamVRSettings)} at {_fallbackResourceDirectory}.");
        HxSteamVRSettings asset = CreateInstance<HxSteamVRSettings>();
        AssetDatabase.CreateAsset(asset, Path.Combine(_fallbackResourceDirectory, 
            $"{HxSteamVRSettings.serializedSingletonName}.asset"));
        AssetDatabase.SaveAssets();
      }
      _serializedSteamVRSettings = new SerializedObject(HxSteamVRSettings.Serialized);
    }

    if (_serializedSteamVRSettings != null) {
      EditorGUILayout.LabelField("SteamVR Settings", EditorStyles.boldLabel);
      HxGUILayout.SerializedFieldLayout(_serializedSteamVRSettings, "setRequiredSteamVRSettingsOnStart");
      HxGUILayout.SerializedFieldLayout(_serializedSteamVRSettings, "suppressChangingSteamVRSettingsWarning");
    } else {
      EditorGUILayout.LabelField(
          "Failed to load serialized HxSteamVRSettings resource. Please let HaptX support know that this has occurred.");
    }
    ...
    if (GUILayout.Button("Open HaptX Unity Settings Window")) {
      HxUnitySettingsWindow.OpenWindow();
    }
  }
